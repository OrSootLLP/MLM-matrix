...
@section('panel')
    <div class="row">
        <div class="col-12">
            <div class="row gy-4">
                <div class="col-xxl-3 col-sm-6 col-xl-4">
                    <x-widget type="2" value="{{ showAmount($user->balance) }}" title="Balance" style="7"
                        link="{{ route('admin.report.transaction', $user->id) }}" icon="las la-money-bill-wave-alt" bg="indigo" />
                </div>

                <div class="col-xxl-3 col-sm-6 col-xl-4">
                    <x-widget type="2" value="{{ showAmount($totalDeposit) }}" title="Deposits" style="7"
                        link="{{ route('admin.deposit.list', $user->id) }}" icon="las la-wallet" bg="8" />
                </div>

                <div class="col-xxl-3 col-sm-6 col-xl-4">
                    <x-widget type="2" value="{{ showAmount($totalWithdrawals) }}" title="Withdrawals" style="7"
                        link="{{ route('admin.withdraw.data.all', $user->id) }}" icon="la la-bank" bg="6" />
                </div>

                <div class="col-xxl-3 col-sm-6 col-xl-4">
                    <x-widget type="2" value="{{ $totalTransaction }}" title="Transactions" style="7"
                        link="{{ route('admin.report.transaction', $user->id) }}" icon="las la-exchange-alt" bg="16" />
                </div>

                <div class="col-xxl-3 col-sm-6 col-xl-4">
                    <x-widget type="2" value="{{ showAmount($user->total_invest) }}" title="Total Invest" style="7"
                        link="{{ route('admin.report.invest') }}?search={{ $user->username }}&exact_match=true" icon="las la-money-bill-wave-alt"
                        bg="17" />
                </div>
                <div class="col-xxl-3 col-sm-6 col-xl-4">
                    <x-widget type="2" value="{{ showAmount($user->total_ref_com) }}" title="Total Referral Commission" style="7"
                        link="{{ route('admin.report.referral.commission') }}?search={{ $user->username }}&exact_match=true" icon="las la-wallet"
                        bg="17" />
                </div>
                <div class="col-xxl-3 col-sm-6">
                    <x-widget type="2" value="{{ showAmount($user->total_binary_com) }}" title="Total Binary Commission" style="7"
                        link="{{ route('admin.report.binary.commission') }}?search={{ $user->username }}&exact_match=true" icon="fas fa-wallet"
                        bg="18" />
                </div>
                <div class="col-xxl-3 col-sm-6">
                    <x-widget link="javascript:void(0)" type="2" value="{{ getAmount($totalBv) }}" title="Total BV" style="7"
                        icon="las la-exchange-alt" bg="4" />
                </div>
            </div>
            ...
            <div class="d-flex mt-4 flex-wrap gap-3">
                ...
                <div class="flex-fill">
                    <a class="btn btn--success btn--shadow w-100 btn-lg" href="{{ @$user->username ?  route('admin.users.binary.tree', $user->username) : "#" }}">
                ...
                <div class="flex-fill">
                    @if ($user->status == Status::USER_ACTIVE)
                        <button class="btn btn--warning w-100 btn-lg userStatus" data-bs-toggle="modal"
                            data-bs-target="#userStatusModal" type="button">
                            <i class="las la-ban"></i>@lang('Ban User')
                        </button>
                    @else
                        <button class="btn btn--success w-100 btn-lg userStatus" data-bs-toggle="modal"
                            data-bs-target="#userStatusModal" type="button">
                        ...
                        <div class="row">
                            ...
                            <div class="col-xl-3 col-md-6 col-12">
                                <div class="form-group">
                                    ...
                                    <input name="ev" data-width="100%" data-onstyle="-success" data-offstyle="-danger" data-bs-toggle="toggle"
                                        data-on="@lang('Verified')" data-off="@lang('Unverified')" type="checkbox"
                            ...
                            <div class="col-xl-3 col-md-6 col-12">
                                <div class="form-group">
                                    ...
                                    <input name="sv" data-width="100%" data-onstyle="-success" data-offstyle="-danger" data-bs-toggle="toggle"
                                        data-on="@lang('Verified')" data-off="@lang('Unverified')" type="checkbox"
...
@push('breadcrumb-plugins')
    <a class="btn btn-sm btn-outline--primary" href="{{ route('admin.users.login', $user->id) }}" target="_blank"><i
